...
function abrirFormularioProducto(prod = null) {
  ...
  <label class="text-xs text-purple-700 font-semibold block mb-2">
    🗓 Quincenal (valor por quincena)
  </label>
  ...
  <input
    ...
    type="number"
    placeholder="$0"
    value="${prod?.preciosCuotas?.[2] || ''}"
    oninput="mostrarTotalCuota(2)"
    class="w-full p-2 border border-purple-300 rounded-lg text-sm focus:outline-none focus:border-purple-500"
  >

  <p id="info2" class="text-xs text-purple-600 mt-1">
    Total de referencia: $${(prod?.preciosCuotas?.[2] || 0) * 2}
  </p>
</div>
...
function mostrarTotalCuota(c) {
  ...
  if (c === 2) {
    el.innerHTML = `Total de referencia (2 quincenas): $${(valor * 2).toLocaleString()}`;
  } else {
...
function saveProductoFinal(id, nombre, foto) {
  const producto = {
    ...
    preciosCuotas: {
      2: Number(document.getElementById('c12').value) || 0,  // Quincenal
...
function pagosHTML() {
  ...
  return `
    ...
    <div class="space-y-5">
      ${pendientes.map(v => {
        const esQuincenal = v.esQuincenal || v.cuotasTotales === 2;
...
function registrarPago(id) {
  ...
  const monto = venta.valorCuota || Math.round(venta.saldo / (venta.cuotasTotales - venta.cuotasPagadas));
  Swal.fire({
    title: 'Registrar pago',
    html: `<div style="text-align:left">
      <p style="margin-bottom:10px;">Monto sugerido: <b>$${monto.toLocaleString()}</b></p>
      <input id="montoPagoInput" type="number" value="${monto}" class="swal2-input" placeholder="Monto">
    </div>`,
    showCancelButton: true, confirmButtonText: 'Registrar', cancelButtonText: 'Cancelar',
    preConfirm: () => {
      const montoIngresado = Number(document.getElementById('montoPagoInput').value);
      if (!montoIngresado || montoIngresado <= 0) { Swal.showValidationMessage('Ingrese un monto válido'); return false; }
    ...
    if (r.isConfirmed) {
      ...
      Swal.fire({
        title: 'Pago registrado', icon: 'success',
        html: `<p>¿Enviar comprobante por WhatsApp?</p>`,
        showCancelButton: true, confirmButtonText: 'Enviar', cancelButtonText: 'No ahora'
      }).then(result => {
